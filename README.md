...
<p align="center">
  <a href="https://github.com/arseniy-io/StreamDock/releases">Скачать</a> ·
  <a href="docs/USER_GUIDE.md">Руководство пользователя</a> ·
  <a href="browser-extension/README.md">Расширение Chrome</a> ·
...
Обычное поле принимает ссылки только на перечисленные поддерживаемые сайты. Расширение Chrome работает иначе: оно находит HLS, DASH и другие медиапотоки внутри уже открытой страницы.
...
Подробная установка и ручной запуск описаны в [docs/INSTALLATION.md](docs/INSTALLATION.md). После запуска используйте [полное руководство пользователя](docs/USER_GUIDE.md). Если что-то не работает, откройте [решение частых проблем](docs/TROUBLESHOOTING.md).
...
После установки откройте страницу с видео и включите воспроизведение на несколько секунд. Затем нажмите значок StreamDock и выберите найденный поток. Расширение сохраняет видео со страницы, а транскрибация запускается после этого в основном приложении. Подробная инструкция, работа с личным кабинетом, разрешения Chrome и ограничения описаны в [руководстве расширения](browser-extension/README.md).
...
### Транскрибировать свой файл

1. Нажмите «Транскрибировать файл с компьютера».
2. Выберите видео или аудио до 20 ГБ.
3. Настройте режим, язык, форматы и дополнительные параметры во вкладке «Текст».
4. Запустите транскрибацию. Файл обрабатывается локально и не отправляется во внешний ИИ-сервис.

### Обработать несколько материалов

Откройте второстепенный блок «Очередь ссылок», вставьте до 50 ссылок или одну ссылку на плейлист и выберите нужные элементы. Очередь может скачать видео, аудио или создать транскрибации. Для текста используются текущие настройки вкладки «Текст». Значения по умолчанию и ограничение настройки очереди описаны в [руководстве пользователя](docs/USER_GUIDE.md).

Все пользовательские настройки находятся в интерфейсе. Редактировать конфигурационные файлы вручную не требуется. Подробное объяснение качества видео, аудиоформатов, моделей, источника текста, временного фрагмента, словаря терминов, спикеров и редактора Markdown находится в [docs/USER_GUIDE.md](docs/USER_GUIDE.md).
...
- Если проект получен через Git, остановите StreamDock, выполните `git pull`, затем запустите `update.bat` и перезагрузите расширение в `chrome://extensions`.
- Если проект получен архивом релиза, остановите StreamDock, сохраните папки `downloads` и `models`, распакуйте новый релиз в постоянную папку, верните эти данные и запустите `install.bat`.
- `update.bat` обновляет зависимости и локальный помощник, но сам не скачивает исходный код.
...
- [docs/USER_GUIDE.md](docs/USER_GUIDE.md) - все пользовательские сценарии и настройки интерфейса;
...
- [browser-extension/README.md](browser-extension/README.md) - установка, использование и безопасность расширения Chrome;
...
- [SUPPORT.md](SUPPORT.md) - куда обращаться с вопросом, ошибкой или уязвимостью.

## Помощь и обратная связь

- Вопросы по использованию задавайте в [Discussions](https://github.com/arseniy-io/StreamDock/discussions).
- Воспроизводимые ошибки оформляйте через [Issues](https://github.com/arseniy-io/StreamDock/issues).
- Сведения об уязвимости отправляйте приватно по правилам [SECURITY.md](SECURITY.md).

Перед публикацией диагностики удалите cookies, приватные ссылки, заголовки авторизации, личные пути и сведения о содержании закрытых материалов.

## Лицензия

Исходный код StreamDock распространяется по лицензии [MIT](LICENSE). Используемые библиотеки, модели и загружаемые материалы могут иметь собственные лицензии и условия использования. Основные сторонние компоненты перечислены в [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md).
